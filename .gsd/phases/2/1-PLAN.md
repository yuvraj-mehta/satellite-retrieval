...
# Plan 2.1: ResNet50 Encoder & Channel Adapter

## Objective

Implement a pretrained ResNet50 feature extractor that handles both modalities. SAR has 2 channels; Optical has 3 (after RGB selection). ResNet50 expects 3-channel input, so we need a channel adapter. Output is 2048-d L2-normalized embeddings — ready for cosine similarity via FAISS dot product.

## Context

- `models/encoder.py` — file to implement
- `datasets/sen12ms_dataset.py` — source of SAR (2-ch) and Optical (3-ch) tensors
- PyTorch 2.12.1, TorchVision 0.27.1 installed
- MPS available on M1

## Tasks

<task type="auto">
  <name>Implement models/encoder.py — ResNet50 feature extractor with channel adapter</name>
  ...
  <verify>
    ...
    python models/encoder.py 2>&1 | tail -10
    # Expected:
    # Device: mps
    # SAR embedding shape: torch.Size([4, 2048])
    # SAR embedding norm: tensor([1., 1., 1., 1.], ...)
    # Optical embedding shape: torch.Size([4, 2048])
  </verify>
  <done>
    - `python models/encoder.py` runs without error
    - Device is `mps` on M1
    - SAR embedding shape: `(4, 2048)`
    - All norms ≈ 1.0 (L2 normalized)
    - Optical embedding shape: `(4, 2048)`
  </done>
</task>

## Success Criteria

- [ ] `models/encoder.py` runs with `if __name__ == "__main__"` test block
- [ ] Both SAR (2-ch) and Optical (3-ch) inputs produce `(B, 2048)` L2-normalized output
- [ ] Device is `mps` on M1
- [ ] Embedding norms are all `1.0` (±1e-5 float precision)
